...
import jimmy.dimension.core.TutDimensions;
import jimmy.dimension.core.TutTabs;
import jimmy.dimension.core.lib.Locations;
import jimmy.dimension.core.travel.TeleporterTutorial;
...
{
	public BlockModPortal(int par1, String texture)
	{
		super(par1, Locations.textureLocation + texture, Material.portal, false);
		this.setTickRandomly(true);
		this.setTextureName(texture);
		this.setHardness(-1.0F);
		this.setStepSound(soundGlassFootstep);
		this.setLightValue(0.75F);
		setCreativeTab(TutTabs.tutBlocks);
	}
	/**
	 ...
	 */
	//Block.coalBlock is my portal frame block
	public static boolean tryToCreatePortal(World par1World, int par2, int par3, int par4)
	{
		byte b0 = 0;
		byte b1 = 0;
		if (par1World.getBlockId(par2 - 1, par3, par4) == Block.coalBlock.blockID || par1World.getBlockId(par2 + 1, par3, par4) == Block.coalBlock.blockID)
		{
			b0 = 1;
		}
		if (par1World.getBlockId(par2, par3, par4 - 1) == Block.coalBlock.blockID || par1World.getBlockId(par2, par3, par4 + 1) == Block.coalBlock.blockID)
		{
			b1 = 1;
		}
		if (b0 == b1)
		...
		else
		{
			...
			for (l = -1; l <= 2; ++l)
			{
				for (i1 = -1; i1 <= 3; ++i1)
				{
					...
					if (l != -1 && l != 2 || i1 != -1 && i1 != 3)
					{
						...
						if (flag)
						{
							if (j1 != Block.coalBlock.blockID)
							{
								return false;
							}
						}
						else if (j1 != 0 && j1 != TutBlocks.tutFire.blockID)
						{
							return false;
						}
					}
			...
			for (l = 0; l < 2; ++l)
			{
				for (i1 = 0; i1 < 3; ++i1)
				{
					par1World.setBlock(par2 + b0 * l, par3 + i1, par4 + b1 * l, TutBlocks.tutPortal.blockID, 0, 2);
				}
			}
			return true;
		}
	...
	public void onNeighborBlockChange(World par1World, int par2, int par3, int par4, int par5)
	{
		...
		if (par1World.getBlockId(par2, i1 - 1, par4) != Block.coalBlock.blockID)
		{
		...
		else
		{
			...
			if (j1 == 3 && par1World.getBlockId(par2, i1 + j1, par4) == Block.coalBlock.blockID)
			{
				...
				{
					if ((par1World.getBlockId(par2 + b0, par3, par4 + b1) != Block.coalBlock.blockID || par1World.getBlockId(par2 - b0, par3, par4 - b1) != this.blockID) && (par1World.getBlockId(par2 - b0, par3, par4 - b1) !=Block.coalBlock.blockID || par1World.getBlockId(par2 + b0, par3, par4 + b1) != this.blockID))
					{
						par1World.setBlockToAir(par2, par3, par4);
					}
				}
	...
	public void onEntityCollidedWithBlock(World par1World, int par2, int par3, int par4, Entity par5Entity)
	{
		if ((par5Entity.ridingEntity == null) && (par5Entity.riddenByEntity == null) && ((par5Entity instanceof EntityPlayerMP)))
		{
			...
			else if (thePlayer.dimension != TutDimensions.tutDimensionID)
			{
				thePlayer.timeUntilPortal = 10;
				thePlayer.mcServer.getConfigurationManager().transferPlayerToDimension(thePlayer, TutDimensions.tutDimensionID, new TeleporterTutorial(thePlayer.mcServer.worldServerForDimension(TutDimensions.tutDimensionID)));
			}
			else {
				thePlayer.timeUntilPortal = 10;
